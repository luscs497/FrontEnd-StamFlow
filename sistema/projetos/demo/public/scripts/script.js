...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  listItems.forEach((element, index) => {
    element.addEventListener("click", () => {
      // CORREÇÃO DEMO: bloqueia a navegação para Relatórios/Insights na
      // versão demo (window.DemoGate só existe no build do demo.stamflow.com.br).
      const title = element.getAttribute("title");
      if (window.DemoGate && !window.DemoGate.canNavigateToSection(title)) {
        return; // paywall já foi mostrado por canNavigateToSection
      }
  ...
  abrirPausaMental.forEach((el) => {
    el.addEventListener("click", () => {
      // CORREÇÃO DEMO: este botão abre o popup direto, sem passar por
      // playTrackByIndex — precisa da mesma checagem aqui.
      if (window.DemoGate && !window.DemoGate.canPlayCategory("mental")) return;
      if (popUpGeral) popUpGeral.classList.remove("display-none");
  ...
  abrirFoco.forEach((el) => {
    el.addEventListener("click", () => {
      // CORREÇÃO DEMO: mesmo caso do abrirPausaMental acima.
      if (window.DemoGate && !window.DemoGate.canPlayCategory("foco")) return;
      if (popUpGeral) popUpGeral.classList.remove("display-none");
  ...
  function playTrackByIndex(categoria, gIndex, iIndex) {
    // CORREÇÃO DEMO: ponto único onde QUALQUER áudio é disparado (clique na
    // lista, popup de Pausa Mental, popup de Foco, próximo/anterior).
    // Bloqueia categorias vedadas e conta exercícios usados no demo.
    if (window.DemoGate && !window.DemoGate.canPlayCategory(categoria)) {
      return; // paywall já foi mostrado por canPlayCategory
    }
