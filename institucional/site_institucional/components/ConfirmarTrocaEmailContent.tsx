...
export function ConfirmarTrocaEmailContent() {
  ...
  useEffect(() => {
    if (!token) {
      setStatus("error");
      setMessage("Link inválido. Solicite a troca de e-mail novamente pelo painel.");
      return;
    }
    fetch(`${API_BASE}/auth/confirm-email-change?token=${encodeURIComponent(token)}`)
      .then(async (res) => {
        ...
        if (res.ok) {
          setStatus("success");
          setMessage(data?.message ?? "E-mail atualizado com sucesso!");
        } else {
          setStatus("error");
      ...
      <div className="relative mx-auto max-w-[42rem] px-6 text-center sm:px-10">
        ...
        <motion.h1
          ...
          className={`font-display text-4xl font-bold sm:text-5xl ${
            status === "success" ? "text-signal" : status === "error" ? "text-raio" : "text-slatey"
          }`}
        >
          {status === "loading" && "Confirmando…"}
          {status === "success" && "E-mail atualizado!"}
        ...
        {status === "success" && (
          ...
          >
            <p className="mb-5 text-[15px] text-muted">
              Use o novo e-mail da próxima vez que fizer login.
            </p>
